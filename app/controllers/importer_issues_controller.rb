...
class ImporterIssuesController < ApplicationController
  unloadable
  ...
  def create
    if params[:attachments].present?
      @import = ImportAction.new(import_params)
  ...
  def update
    ...
    if @import.all_required_fields_are_mapped?
      redirect_to overview_project_importer_issue_path(@project, @import)
    else
  ...
  def change_file_recover
    ...
    if params[:attachments].present?
      ...
      if !@import.check_file
        flash[:error] = l(:error_file_incorrect)
        redirect_to recover_project_importer_issue_path(@project, @import)
      else      
        redirect_to overview_project_importer_issue_path(@project, @import)
      end 
    else
  ...
  def import_params
    params.require(:import).permit(:id, :user_id, :project_id, :status, :created_at, :tracker_id, :headers, :log, :is_template, :name, :description, :parent_id, :fields, :field_for_update, :template_id, :time_entry_fields)
  end
